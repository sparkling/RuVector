...
use super::zkproofs::{
    FinancialProofBuilder, ProofType, RangeProof, RentalApplicationProof, VerificationResult,
    ZkProof,
};
...
impl ZkFinancialProver {
    ...
    pub fn load_income(&mut self, monthly_income: Vec<u64>) {
        self.builder = std::mem::take(&mut self.builder).with_income(monthly_income);
    }
    ...
    pub fn load_expenses(&mut self, category: &str, monthly_expenses: Vec<u64>) {
        self.builder = std::mem::take(&mut self.builder).with_expenses(category, monthly_expenses);
    }
    ...
    pub fn load_balances(&mut self, daily_balances: Vec<i64>) {
        self.builder = std::mem::take(&mut self.builder).with_balances(daily_balances);
    }
    ...
    pub fn prove_debt_ratio(
        &self,
        monthly_debt_cents: u64,
        max_ratio_percent: u64,
    ) -> Result<JsValue, JsValue> {
        self.builder
            .prove_debt_ratio(monthly_debt_cents, max_ratio_percent)
            .map(|proof| serde_wasm_bindgen::to_value(&proof).unwrap())
            .map_err(|e| JsValue::from_str(&e))
    }
...
impl ZkUtils {
    ...
    pub fn proof_from_url(encoded: &str) -> Result<String, JsValue> {
        ...
        String::from_utf8(decoded).map_err(|e| JsValue::from_str(&format!("Invalid UTF-8: {}", e)))
    }
}
