...
fn main() {
    ...
    for (i, pkg) in packages.iter().enumerate() {
        ...
        let meta = vec![
            MetadataEntry {
                field_id: 1,
                value: MetadataValue::String(pkg.to_string()),
            },
            MetadataEntry {
                field_id: 2,
        ...
        store
            .ingest_batch(&[vec.as_slice()], &[i as u64], Some(&meta))
            .expect("ingest");
    }
    ...
    for i in packages.len()..num_vectors {
        let vec = random_vector(dim, i as u64);
        store
            .ingest_batch(&[vec.as_slice()], &[i as u64], None)
            .expect("ingest");
    }

    println!(
        "  [VEC_SEG]     {} vectors ingested ({}-dim, cosine)",
        num_vectors, dim
    );
    ...
    let initramfs = builder
        .build_initramfs(&["sshd", "rvf-server"], &[])
    ...
    let kernel_label = if kernel.bzimage.len() > 8192 {
        "real bzImage"
    } else {
        "builtin stub"
    };
    ...
    store
        ...
        .expect("embed kernel");
    println!(
        "  [KERNEL_SEG]  Embedded with api_port:2222, cmdline:'{}'",
        cmdline
    );

    // Embed eBPF
    let ebpf =
        EbpfCompiler::from_precompiled(EbpfProgramType::XdpDistance).expect("precompiled ebpf");
    store
        .embed_ebpf(
        ...
        .expect("embed ebpf");
    println!(
        "  [EBPF_SEG]    {} bytes (XDP distance, precompiled ELF)",
        ebpf.elf_bytes.len()
    );
    ...
    let entries = vec![
        WitnessEntry {
            ...
            action_hash: shake256_256(
                format!("ingest:{} vectors, dim {}", num_vectors, dim).as_bytes(),
            ),
    ...
    println!(
        "  [WITNESS_SEG] {} entries, chain verified",
    ...
    let results = store
        ...
        .expect("query");
    println!(
        "  [QUERY]       Top-5 neighbors for 'claude-code': {:?}",
    ...
    if let Some((kh_bytes, kdata)) = store.extract_kernel().expect("extract kernel") {
        println!(
            "  Kernel:       {} bytes header, {} bytes image",
            kh_bytes.len(),
            kdata.len()
        );
    }
    if let Some((eh_bytes, edata)) = store.extract_ebpf().expect("extract ebpf") {
        println!(
            "  eBPF:         {} bytes header, {} bytes program",
            eh_bytes.len(),
            edata.len()
        );
    }
    ...
    let results2 = store
        .query(&query_vec, 3, &QueryOptions::default())
        .expect("query");
    println!(
        "  Query verify: IDs {:?} (consistent: {})",
    ...
    match start {
        Ok(container_id) => {
            ...
            if let Ok(ssh_check) = docker_run(&[
                "exec",
                container_name,
                "sh",
                "-c",
                ...
                println!(
                    "    ssh-listen:   port 22222 {}",
                    if ssh_check.contains("22222") {
                        "OPEN"
                    } else {
                        "checking..."
                    }
            ...
            if let Ok(size) = docker_run(&["exec", container_name, "sh", "-c", "wc -c < /data.rvf"])
            {
                let inner_size: u64 = size.trim().parse().unwrap_or(0);
                println!(
                    "    rvf-size:     {} bytes (match: {})",
                    inner_size,
            ...
            if let Ok(magic_check) = docker_run(&[
                "exec",
                container_name,
                "sh",
                "-c",
                ...
                println!(
                    "    rvf-magic:    {} (RVFS)",
                    if has_magic { "VALID" } else { "checking..." }
                );
            }
    ...
    println!(
        "  Witness:      {} entries, hash chain verified",
        verified_entries.len()
    );
